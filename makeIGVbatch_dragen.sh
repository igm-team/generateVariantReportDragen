...
while read i;
    do
       ...
       /nfs/goldstein/software/jdk1.8.0_05/bin/java -Xmx24g -jar /nfs/goldstein/software/GATK/GATK-3.6.0-ArchivedVersion-g89b7209-patched/GenomeAnalysisTK.jar -R /scratch/HS_Build37/BWA_INDEX_hs37d5/hs37d5.fa -T HaplotypeCaller -L $VAR_CHR:$((VAR_LOC-500))-$((VAR_LOC+500)) -I $PRO_LOC -o $(dirname $IN)/BAMS/$(echo $i|awk    '{print $1"."$6"."$7".vcf.gz"}') -stand_call_conf 20 -stand_emit_conf 20 --emitRefConfidence GVCF -GQB 5 -GQB 15 -GQB 20 -GQB 60 --variant_index_type LINEAR --variant_index_parameter 128000 --dbsnp /nfs/goldstein/goldsteinlab/software/GATK_bundle_2.8_b37/dbsnp_138.b37.vcf.gz -nct 1 --bamOutput $(dirname $IN)/BAMS/$(echo $i|awk   '{print $2"."$6"."$7".bam"}')
       /nfs/goldstein/software/jdk1.8.0_05/bin/java -Xmx24g -jar /nfs/goldstein/software/GATK/GATK-3.6.0-ArchivedVersion-g89b7209-patched/GenomeAnalysisTK.jar -R /scratch/HS_Build37/BWA_INDEX_hs37d5/hs37d5.fa -T HaplotypeCaller -L $VAR_CHR:$((VAR_LOC-500))-$((VAR_LOC+500)) -I $DAD_LOC -o $(dirname $IN)/BAMS/$(echo $i|awk    '{print $3"."$6"."$7".vcf.gz"}') -stand_call_conf 20 -stand_emit_conf 20 --emitRefConfidence GVCF -GQB 5 -GQB 15 -GQB 20 -GQB 60 --variant_index_type LINEAR --variant_index_parameter 128000 --dbsnp /nfs/goldstein/goldsteinlab/software/GATK_bundle_2.8_b37/dbsnp_138.b37.vcf.gz -nct 1  --bamOutput $(dirname $IN)/BAMS/$(echo $i|awk   '{print $3"."$6"."$7".bam"}')
       ...
       /nfs/goldstein/software/jdk1.8.0_05/bin/java -Xmx24g -jar /nfs/goldstein/software/GATK/GATK-3.6.0-ArchivedVersion-g89b7209-patched/GenomeAnalysisTK.jar -R /scratch/HS_Build37/BWA_INDEX_hs37d5/hs37d5.fa -T HaplotypeCaller -L $VAR_CHR:$((VAR_LOC-500))-$((VAR_LOC+500)) -I $MOM_LOC -o $(dirname $IN)/BAMS/$(echo $i|awk    '{print $1"."$6"."$7".vcf.gz"}') -stand_call_conf 20 -stand_emit_conf 20 --emitRefConfidence GVCF -GQB 5 -GQB 15 -GQB 20 -GQB 60 --variant_index_type LINEAR --variant_index_parameter 128000 --dbsnp /nfs/goldstein/goldsteinlab/software/GATK_bundle_2.8_b37/dbsnp_138.b37.vcf.gz -nct 1 --bamOutput $(dirname $IN)/BAMS/$(echo $i|awk   '{print $4"."$6"."$7".bam"}')
...
find $(pwds)/$(dirname $IN)/BAMS -name '*.bam'|sed 's/\.\///'|sed 's/home\/[^\/]*/\/10.73.50.80\/homes/'|sed 's/\/nfs/\\\\10.73.50.80/' |sed 's/homes\/svaprojects/homes/'|sed 's/10.73.50.80\/fastq\([0-9]\+\)/igm-avere.igm.cumc.columbia.edu\/fastq\1/' | sed 's/\//\\/g'|sed 's/\\\\ALIGNMENT/\\ALIGNMENT/g' > $(dirname $IN)/$(basename ${IN%.*}).bamwinloc
