...
Filter.for.tier2.general <- function(data) {
  ...
  columns <- c("HGMD Variant Class","ClinVar Clinical Significance",
               "Function","HGMD Flanking Count","ClinVar Flanking Count",
               "ClinGen HaploinsufficiencyDesc","ClinVar Pathogenic Indel Count",
  ...
  #inclusion rule 3:
  suppressWarnings(temp <- sapply(data[normalized.name("ClinVar Flanking Count")], as.numeric))
  temp[is.na(temp)] <- 0
  R3 <- Functional & (temp > 0)

  #inclusion rule 4:
  suppressWarnings(temp <- sapply(data[normalized.name("ClinGen HaploinsufficiencyDesc")], as.numeric))
  temp[is.na(temp)] <- 0
...
Filter.for.tier2.pdnm.kv <- function(data) {
  ...
  columns <- c("ExAC global gts","Rms Map Qual MQ", "Qual By Depth QD", "Qual","Gatk Filtered Coverage",
               "Percent Alt Read","Function", "HGMD Variant Class", "ClinVar Flanking Count", "HGMD Flanking Count", "ClinVar Clinical Significance"
  )
  ...
  #inclusion rule 2:
  temp<- sapply(data[normalized.name("HGMD Flanking Count")], as.character)
  R21 <- temp == 0
  temp<- sapply(data[normalized.name("ClinVar Flanking Count")], as.character)
  R22 <- temp == 0
  R2 <- !(R21 & R22)
...
Filter.for.tier2.pdnm.lof <- function(data) {
  ...
  #check for correct columns
  columns <- c("Function", "HGMD Variant Class", "HGMD Flanking Count", "ClinVar Flanking Count",
               "ClinVar Clinical Significance","ClinGen HaploinsufficiencyDesc", "ClinVar Pathogenic Indel Count",
  ...
  #inclusion rule 2:
  suppressWarnings(temp <- sapply(data[normalized.name("HGMD Flanking Count")], as.numeric))
  temp[is.na(temp)] <- 0
  R21 <- temp > 0
  ...
  #inclusion rule 4:
  R4 <- (!(eval(parse(text=paste0("data$",normalized.name("ClinGen HaploinsufficiencyDesc")))) %in%  c("No evidence","Little evidence") | is.na(eval(parse(text=paste0("data$",normalized.name("ClinGen HaploinsufficiencyDesc"))))))) 
...
Filter.for.tier2.prec.kv.lof <- function(data) {
  ...
  #check for correct columns
  columns <- c("Genotype","Function", "HGMD Variant Class", "HGMD Flanking Count", "ClinVar Flanking Count",
               "ClinVar Clinical Significance","ClinGen HaploinsufficiencyDesc", "ClinVar Pathogenic Indel Count",
               "ClinVar Pathogenic SNV Splice Count","ClinVar Pathogenic SNV Nonsense Count",
               "Clinvar Pathogenic CNV Count")
