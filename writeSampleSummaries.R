...
writeDNM <- function(dnm,rtf){
    ...
    for(i in 1:dim(dnm)[1]){
        ...
        if((dnm[i,]$Ctrl.AF == 0 | is.na(dnm[i,]$Ctrl.AF)) & (dnm[i,]$Evs.All.Maf == 0 | is.na(dnm[i,]$Evs.All.Maf)) & (dnm[i,]$ExAC.global.af == 0 | is.na(dnm[i,]$ExAC.global.af)) & (dnm[i,]$gnomAD.Exome.global.maf == 0 | is.na(dnm[i,]$gnomAD.Exome.global.maf))){
            addText(rtf,"This variant is absent from internal and external control samples. ")}
        else{addText(rtf,paste0("This variant has a control AF of ",dnm[i,]$Ctrl.AF," in IGM controls, ",dnm[i,]$Evs.All.Maf," in EVS, ",dnm[i,]$ExAC.global.af," in ExAC, and ",dnm[i,]$gnomAD.Exome.global.maf," in gnomAD. "))}
        ...
        if(!is.na(dnm[i,]$ClinVar.Clinical.Significance)){
           addText(rtf,paste0("This variant is listed as ",tolower(dnm[i,]$ClinVar.Clinical.Significance)," in ClinVar for ",dnm[i,]$ClinVar.Disease,". "))}
...
#First variant
        if((chet[i,]$Ctrl.AF.1 == 0 | is.na(chet[i,]$Ctrl.AF.1)) & (chet[i,]$Evs.All.Maf.1 == 0 | is.na(chet[i,]$Evs.All.Maf.1)) & (chet[i,]$ExAC.global.af.1 == 0 | is.na(chet[i,]$ExAC.global.af.1)) & (chet[i,]$gnomAD.Exome.global.maf.1 == 0 | is.na(chet[i,]$gnomAD.Exome.global.maf.1))){
            addText(rtf,"The first variant is absent from internal and external control samples. ")}
        else{addText(rtf,paste0("The first variant has a control AF of ",chet[i,]$Ctrl.AF.1," in IGM controls, ",chet[i,]$Evs.All.Maf.1," in EVS, ",chet[i,]$ExAC.global.af.1," in ExAC, "),chet[i,]$gnomAD.Exome.global.maf.1," in gnomAD. ")}
        ...
        if(!is.na(chet[i,]$ClinVar.Clinical.Significance.1)){
           addText(rtf,paste0("This variant is listed as ",tolower(chet[i,]$ClinVar.Clinical.Significance.1)," in ClinVar for",chet[i,]$ClinVar.Disease.1,". "))}
        ...
        #second variant
        addText(rtf,"\n")
        if((chet[i,]$Ctrl.AF.2 == 0 | is.na(chet[i,]$Ctrl.AF.2)) & (chet[i,]$Evs.All.Maf.2 == 0 | is.na(chet[i,]$Evs.All.Maf.2)) & (chet[i,]$ExAC.global.af.2 == 0 | is.na(chet[i,]$ExAC.global.af.2)) & (chet[i,]$gnomAD.Exome.global.maf.2 == 0 | is.na(chet[i,]$gnomAD.Exome.global.maf.2))){
            addText(rtf,"The second variant is absent from internal and external control samples. ")}
        else{addText(rtf,paste0("The second variant has a control AF of ",chet[i,]$Ctrl.AF.2," in IGM controls, ",chet[i,]$Evs.All.Maf.2," in EVS, ",chet[i,]$ExAC.global.maf.2," in ExAC, ",chet[i,]$gnomAD.Exome.global.maf.2," in gnomAD. "))}
        ...
        if(!is.na(chet[i,]$ClinVar.Clinical.Significance.2)){
           addText(rtf,paste0("This variant is listed as ",tolower(chet[i,]$ClinVar.Clinical.Significance.2)," in ClinVar for ",chet[i,]$ClinVar.Disease.2,". "))}
